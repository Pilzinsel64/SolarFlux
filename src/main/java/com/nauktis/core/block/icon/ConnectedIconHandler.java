...
public class ConnectedIconHandler implements IBlockIconHandler {
    ...
    private final ForgeDirection[] SIDE_UP = { ForgeDirection.EAST, ForgeDirection.NORTH, ForgeDirection.UP,
        ForgeDirection.UP, ForgeDirection.UP, ForgeDirection.UP };
    private final ForgeDirection[] SIDE_RIGHT = { ForgeDirection.SOUTH, ForgeDirection.EAST, ForgeDirection.EAST,
        ForgeDirection.EAST, ForgeDirection.EAST, ForgeDirection.EAST };
    private final ForgeDirection[] SIDE_DOWN = { ForgeDirection.WEST, ForgeDirection.SOUTH, ForgeDirection.DOWN,
        ForgeDirection.DOWN, ForgeDirection.DOWN, ForgeDirection.DOWN };
    private final ForgeDirection[] SIDE_LEFT = { ForgeDirection.NORTH, ForgeDirection.WEST, ForgeDirection.WEST,
    ...
    public IIcon getIcon(IBlockAccess pBlockAccess, int pX, int pY, int pZ, int pSide) {
        ...
        if (shouldConnect(
            ...
            pY,
            pZ,
            pX + neighborDirection.offsetX,
            pY + neighborDirection.offsetY,
            pZ + neighborDirection.offsetZ)) {
            textureIndex += 1;
        }
    ...
    private boolean shouldConnect(IBlockAccess pBlockAccess, int pX, int pY, int pZ, int pCandidateX, int pCandidateY,
        int pCandidateZ) {
        return pBlockAccess.getBlock(pX, pY, pZ)
            .equals(pBlockAccess.getBlock(pCandidateX, pCandidateY, pCandidateZ));
    }
